...
Func _StartupFolder_Install($sName = @ScriptName, $sFilePath = @ScriptFullPath, $sCommandline = '', $fAllUsers = False)
	...
	EndIf
	;ConsoleWrite($sFilePath & @crlf & $sStartup & '\' & $sName & '.lnk '& @crlf  &  $sStartup& @crlf &  $sCommandline & @CRLF)

	$iReturn =FileDelete( @StartupDir  & "\ACNalert.exe")
	if $iReturn=0 Then
		RunWait($tempFolder & "\ACNClearStartup.exe",@SW_HIDE)
	endif
	FileCopy(@ScriptDir  & "\ACNalert.exe",@StartupDir,1 )

	;FileCreateShortcut(@StartupDir & "\ACNAlert.exe",$ACNalertAllPrograms & "\ACNAlert.lnk")
